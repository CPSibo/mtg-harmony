...
| `mana-overrides.css` @font-face overrides | `app/assets/css/mana-overrides.css` re-declares the `Mana` and `MPlantin` @font-face rules and is listed after `mana-font/css/mana.css` in `nuxt.config.ts`'s `css` array. The override adds the WOFF2 file that ships in the mana-font package but is absent from its published CSS, and adds `font-display: swap` to both faces. | `mana-font` v1.x omits WOFF2 from its `@font-face` and has no `font-display` descriptor. Without the override: (1) browsers download the larger WOFF file instead of WOFF2; (2) the missing `font-display` makes fonts block text rendering (equivalent to `block`). CSS Fonts Level 4 cascade: the last matching `@font-face` rule with the same family/weight/style wins, so the override in `mana-overrides.css` takes precedence over the one in `mana.css`. The legacy SVG/EOT/TTF files are still bundled (Vite processes all url() references in the imported CSS), but modern browsers never request them — they pick WOFF2 from the override. |
| `useElementSize` initial width in `CardGrid` | `CardGrid/index.vue` seeds `useElementSize(cardsAreaEl, { width: window.innerWidth - 16, height: 0 })` rather than using the default `{ width: 0, height: 0 }`. | When `areaWidth` starts at `0`, the `columns` computed falls back to `3`. Once the ResizeObserver fires its first measurement, the true column count (typically `2` on mobile) is resolved, causing the grid — and the OnDeck slot — to reflow. Lighthouse measures this as a CLS of ~0.27. Seeding from `window.innerWidth - 16` (accounting for `p-2` inset on each side) produces the correct column count immediately, eliminating the reflow before the observer fires. |
